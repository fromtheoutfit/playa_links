...
if (sizeof($children) > 0)
{
    ...
    foreach ($children as $channel => $entries)
    {
        ...
        {
            $this->table->add_row('<a href="' . html_entity_decode(BASE) . AMP . 'C=content_publish' . AMP . 'M=entry_form' . AMP . 'channel_id=' . $entry['channel_id'] . '&entry_id=' . $entry['entry_id'] . '" target="_blank">' . $entry['title'] . '</a>');
        }
    }
...
if (sizeof($parents) > 0)
{
    ...
    foreach ($parents as $channel => $entries)
    {
        ...
        foreach ($entries as $entry)
        {
            $this->table->add_row('<a href="" target="_blank">' . $entry['title'] . '</a>');
        }
    }
